...
#include "panel_ae112_p_3_a0025_dsi_vdo.h"
...
static int lcm_panel_poweron(struct drm_panel *panel)
{
	...
	pr_info("%s: AE112 lcm ctx->prepared %d\n", __func__, ctx->prepared);
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	unsigned int fp_type = 0x210;
	int ret;

	pr_info("[LCM] ae112_p_3_a0025 %s+\n", __func__);
	...
	pr_info("%s, remote_node->full_name = %s, dev->of_node->full_name = %s\n",
			__func__,remote_node->full_name,(dev->of_node)->full_name);
...
static const struct of_device_id lcm_of_match[] = {
	{ .compatible = "panel_ae112_p_3_a0025_dsi_vdo", },
	{ }
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "panel_ae112_p_3_a0025_dsi_vdo",
		.owner = THIS_MODULE,
